...
from dnsrules.inventory import InvalidInventory, load
...
def test_load_reads_the_groups(inventory_path):
    entries = load(inventory_path)
...
def test_load_maps_each_address_to_a_host_name(inventory_path):
    entries = load(inventory_path)
    assert entries.names == {"10.0.0.2": "clove", "100.71.4.9": "clove"}
    assert entries.hosts[0].groups == ("kids",)


def test_a_missing_file_is_an_error_not_an_empty_inventory(tmp_path):
    """An empty inventory would render every zone file with no rules in it."""
    with pytest.raises(InvalidInventory, match="does not exist"):
...
def test_a_group_needs_every_field(tmp_path, groups, message):
    path = write(tmp_path / "inventory.yml", {"groups": groups})
    with pytest.raises(InvalidInventory, match=message):
        load(path)
...
def test_a_zone_name_with_whitespace_is_refused(tmp_path):
    ...
    with pytest.raises(InvalidInventory, match="whitespace"):
        load(path)
...
def test_addresses_must_be_strings(tmp_path):
    ...
    with pytest.raises(InvalidInventory, match="addresses"):
        load(path)


def test_an_inventory_with_no_groups_loads(tmp_path):
    entries = load(write(tmp_path / "inventory.yml", {"groups": [], "hosts": []}))
