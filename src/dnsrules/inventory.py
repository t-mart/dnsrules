"""Read the inventory that Ansible renders.

Ansible owns host names, addresses, group names, and membership. It writes
`/etc/dnsrules/inventory.yml` at deploy time. dnsrules reads that file and
never writes it.
...
class InvalidInventory(ValueError):
    """The inventory is absent, or it does not match the format."""
...
def _object(entry: object, path: Path) -> dict:
    if not isinstance(entry, dict):
        raise InvalidInventory(f"{path} holds an entry that is not an object.")
    return entry
...
def _field(entry: dict, key: str, path: Path) -> str:
    ...
    if not isinstance(value, str) or not value:
        raise InvalidInventory(f"{path} holds an entry with no {key}.")
    return value
...
def _strings(entry: dict, key: str, path: Path) -> tuple[str, ...]:
    ...
    if not isinstance(values, list) or not all(isinstance(v, str) for v in values):
        raise InvalidInventory(f"{path}: {key} must be a list of strings.")
    return tuple(values)
...
def _group(raw: object, path: Path) -> Group:
    ...
    if zone.split() != [zone]:
        raise InvalidInventory(f"{path}: the zone name {zone!r} holds whitespace.")
    return Group(
...
def load(path: Path) -> Inventory:
    """Read the inventory. Raises InvalidInventory for anything unusable.

    An absent file is an error, not an empty inventory. Empty would render
    every zone file with no rules in it.
    """
    try:
        ...
        raise InvalidInventory(
            ...
            f"DNSRULES_INVENTORY_PATH to point elsewhere."
        ) from error
    except yaml.YAMLError as error:
        raise InvalidInventory(f"{path} is not valid YAML: {error}") from error
    if not isinstance(raw, dict):
        raise InvalidInventory(f"{path} does not hold an object.")

    groups: dict[str, Group] = {}
    for entry in raw.get("groups", []):
        group = _group(entry, path)
        if group.name in groups:
            raise InvalidInventory(f"{path} names the group {group.name} twice.")
    ...
    return Inventory(groups=groups, hosts=hosts, names=names)
